<x-app-layout>
    ...
    <div class="col-md-12">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item"><a href="{{route('contabilidad.grupoResultado.index', ['utilidad_id' => $utilidadSeleccionada->id])}}">Grupos de {{$utilidadSeleccionada->utilidad}}</a></li>
            <li class="breadcrumb-item"><a href="{{route('contabilidad.subGrupoResultado.index', ['utilidad_id' => $utilidadSeleccionada->id, 'grupo_id' => $grupo->id ])}}">Sub grupos de {{$grupo->grupo}}</a></li>
            <li class="breadcrumb-item">Cuentas del sub grupo {{$subGrupo->sub_grupo}}</li>
        </ol>
    </div>
    <div class="col-md-12">
        <x-alert></x-alert>
        ...
        <div class="card">
            <div class="card-body">
                ...
                <div class="mt-2 mb-3">
                    ...
                    @include('contabilidad.estado_resultado.cuentaResultado.create')
                    @include('contabilidad.estado_resultado.cuentaResultado.edit')
                </div>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="text-center">
                                ...
                                data-sub_grupo_id="{{ $item->sub_grupo_id }}" 
                                data-cuenta-id="{{ $item->cuenta_id}}" 
                                data-id="{{ $item->id }}"
                                data-utilidad-id="{{ $item->utilidad_id }}">
                                 <i class="fas fa-edit fa-lg"></i>
                                </a>
                                <form id="form{{ $item->id }}"
                                    action="{{ route('contabilidad.cuentaResultado.destroy', ['utilidad_id'=>$utilidadSeleccionada->id,'grupo_id'=>$grupo->id, $subGrupo->id, $item->id]) }}" method="post"
                                    class="d-inline p-1">
                                    @method('DELETE')
                                    @csrf
                                    <a href="#"
