<div class="card">
    <div class="card-body">
        <div class="row">
            <div class="col-md-4">
                <form method="post" action="{{ route('producto.guardarTipoPrecioCombo', ['id' => $data?->id ?? 0]) }}">
                    @csrf
                    <div class="row">
                                ...
                                @foreach ($data->tiposPrecios as $key => $item)
                                    ...
                                    <td>
                                        @if ($item->pivot->estado == true)
                                            <a href="{{ route('producto.estadoTipoPrecioCombo', $data->id) }}?tipo_id={{ $item->id }}&estado={{ $item->pivot->estado }}"
                                    ...
                                    <td>
                                        ...
                                        @if (!$validate)
                                            <a href="{{ route('producto.eliminarTipoPrecioCombo', $data->id) }}?tipo_id={{ $item->id }}"
