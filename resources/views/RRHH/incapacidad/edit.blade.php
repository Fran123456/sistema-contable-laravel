<x-app-layout>
    ...
    <div class="col-md-12">
        <form action="{{ route('rrhh.incapacidad.update', $incapacidad->id ) }}" method="post">
            @method('PUT')
            @csrf
            <div class="card">
                <div class="card-body">
                    ...
                    <div class="row">
                            ...
                            <div class=" col-md-6 mt-2 mb-12 ">
                                ...
                                <select name="empleado_id" id="empleado_id" class=" chosen-select form-select " required>
                                    @foreach ($empleados as $key => $item )
                                        ...
                                        ( old('empleado_id') == null && $incapacidad->empleado_id == $item->id )
                                        || old('empleado_id') == $item->id
                                        || $loop->first ) selected @endif>{{ $item->codigo }} - {{ $item->nombre_completo }}</option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div class=" col-md-6 mt-2 mb-12 ">
                                ...
                                <select name="periodo_planilla_id" id="periodo_planilla_id" class="form-select" required>
                                    @foreach ($periodosPlanillas as $key => $item )
                                        <option value="{{ $item->id }}" @if(
                                        ( old('periodo_planilla_id') == null && $incapacidad->periodo_planilla_id == $item->id )
                                        || old('periodo_planilla_id') == $item->id
                                        || $loop->first ) selected @endif>{{ $item->mes_string }} - {{ $item->year }} {{ $item->tipo_periodo }} {{ $item->periodo_dias }}</option>
                                    @endforeach
                                </select>
                            </div>
                        ...
                        <div class=" row ">
                            ...
                            <div class=" col-md-4 mt-2 mb-12 ">
                                ...
                                <input type="date" name="fecha_inicio" id="fecha_inicio" value="{{ old('fecha_inicio') ?: $incapacidad->fecha_inicio }}" class="form-control" required />
                            </div>
                            ...
                            <div class=" col-md-4 mt-2 mb-12 ">
                                ...
                                <select name="tipo_incapacidad_id" id="tipo_incapacidad_id" class="form-select" required>
                                    @foreach ($tipoIncapacidades as $key => $item )
                                        <option value="{{ $item->id }}" @if(
                                        ( old('tipo_incapacidad_id') == null && $incapacidad->tipo_incapacidad_id == $item->id )
                                        || old('tipo_incapacidad_id') == $item->id
                                        || $loop->first ) selected @endif>{{ $item->tipo }}</option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div class=" col-md-4 mt-2 mb-12 ">
                                ...
                                <input class="form-control" name="cantidad" id="cantidad" type="number" min="1" value="{{ old('cantidad') ?: $incapacidad->cantidad }}" required>
                            </div>
