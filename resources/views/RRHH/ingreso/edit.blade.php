<x-app-layout>
    ...
    <div class="col-md-12 mb-3">
        <x-badge titulo="Editar ingreso" icono="fas fa-user-plus"></x-badge>
    </div>
    <div class="col-md-12">
        <form action="{{ route('rrhh.ingreso.update', $ingreso->id) }}" method="post" enctype="form-data">
            @method('PUT')
            @csrf
            <div class="card">
                <div class="card-body">
                                ...
                                <select name="empleado" id="empleado" class=" chosen-select form-select " required>
                                    @foreach ($empleados as $key => $item)
                                        <option value="{{ $item->id }}"
                                            @if ((old('empleado') == null && $ingreso->id_empleado == $item->id) || old('empleado') == $item->id) selected @endif>{{ $item->codigo }} -
                            ...
                            <div class=" col-md-4 mt-2 mb-12 ">
                                ...
                                <select name="planilla" id="planilla" class=" chosen-select form-select " required>
                                    @foreach ($planillas as $key => $item)
                                        <option value="{{ $item->id }}"
                                            @if ((old('planilla') == null && $ingreso->id_periodo_planilla == $item->id) || old('planilla') == $item->id) selected @endif>{{ $item->mes_string }} -
                            ...
                            <div class=" col-md-4 mt-2 mb-12 ">
                                ...
                                <select name="tipo_ingreso" id="tipo_ingreso" class=" chosen-select form-select "
                                    ...
                                    @foreach ($tiposIngreso as $key => $item)
                                        <option value="{{ $item->id }}"
                                            @if ((old('tipo_ingreso') == null && $ingreso->id_tipo_ingreso == $item->id) || old('tipo_ingreso') == $item->id) selected @endif>{{ $item->tipo }}
                                        </option>
                                    @endforeach
                                </select>
                            </div>
                        ...
                        <div class=" row ">
                            <div class=" col-md-6 mt-2 mb-12 ">
                                ...
                                <input class="form-control" id="cantidad" name="cantidad" type="number"step="0.01"
                                    min="0" value="{{ old('salario') ?: $ingreso->cantidad }}" required>
                            </div>
                            <div class=" col-md-6 mt-2 mb-12 ">
                                <label for="fecha">Fecha</label><span class="text-danger">*</span>
                                <input type="date" name="fecha" id="fecha"
                                    value="{{ old('fecha') ?: $ingreso->fecha }}" class="form-control" required />
                            </div>
                        </div>
                        ...
                        <div class=" row ">
                            <div class=" col-md-8 mt-2 mb-12 ">
                                ...
                                <input name="descripcion" id="descripcion"
                                    value="{{ old('descripcion') ?: $ingreso->descripcion }}" type="text"
                                    class="form-control">
                            </div>
                        </div>
